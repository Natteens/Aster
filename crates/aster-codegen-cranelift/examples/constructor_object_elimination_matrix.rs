//! Release-build evidence for constructor-aware local-object elimination.
...
impl Shape {
    fn name(self) -> &'static str {
        match self {
            Self::OneField => "one-field",
            Self::TwoField => "two-field",
            Self::Mixed => "mixed-occasional",
            Self::Effectful => "effectful-negative",
            Self::Escaping => "escaping-negative",
    ...
    fn source(self, iterations: usize) -> String {
        match self {
            ...
            Self::TwoField => format!(
                "public class Pair {{ public int left; public int right; \
                 public Pair(int left, int right) {{ this.right = right; this.left = left; }} }} \
                 public int Run() {{ int total = 0; for (int i = 0; i < {iterations}; i++) {{ \
                 Pair pair = new Pair(1, 2); total += pair.left + pair.right; }} return total; }}"
            ),
            Self::Mixed => format!(
                "public class Box {{ public int value; public Box(int value) {{ this.value = value; }} }} \
                 public int Run() {{ int total = 0; for (int i = 0; i < {iterations}; i++) {{ \
                 total += i % 7; if (i % 16 == 0) {{ Box box = new Box(5); \
                 total += box.value; }} }} return total; }}"
            ),
            Self::Effectful => format!(
                "internal int Normalize(int value) {{ return value; }} \
                 public class Box {{ public int value; public Box(int value) {{ \
                 this.value = Normalize(value); }} }} public int Run() {{ int total = 0; \
                 for (int i = 0; i < {iterations}; i++) {{ Box box = new Box(1); \
                 total += box.value; }} return total; }}"
            ),
            Self::Escaping => format!(
                "public class Box {{ public int value; public Box(int value) {{ this.value = value; }} }} \
                 public int Run() {{ int total = 0; for (int i = 0; i < {iterations}; i++) {{ \
                 Box box = new Box(1); Box alias = box; total += alias.value; }} return total; }}"
            ),
    ...
    fn expected(self, iterations: usize) -> i32 {
        ...
        match self {
            Self::OneField | Self::Effectful | Self::Escaping | Self::HiddenBacking => iterations,
            Self::TwoField => iterations * 3,
            Self::Mixed => {
...
fn run(shape: Shape, iterations: usize) -> (f64, MemoryStats, usize, u64) {
    ...
    assert_eq!(value, expected);
    let dynamic_regions = telemetry.temporary.events.rewind_events.saturating_sub(1);
    (
        samples[SAMPLES / 2],
        stats,
...
fn main() {
    for iterations in [100_000, 1_000_000] {
        for shape in [
            ...
            println!(
                "mode=optimized shape={:<18} iterations={iterations:<7} median_ms={median_ms:>9.3} checksum={:>8} allocations={:>8} object_allocations={:>8} requested_bytes={:>10} peak_used_bytes={:>10} capacity_bytes={:>10} static_markers={static_markers:>2} dynamic_regions={dynamic_regions:>8}",
                shape.name(),
